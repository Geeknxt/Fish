...
import random, math, pygame, sys, copy
...
class Brain:
    ...
    def __init__(self, numNeurons, density, brain = [], connectome = []):
        ...
        if not self.connectome:
            #for each neuron
            for i in range(0, self.numNeurons):
                #make a random list of dendrites
                connections = []
...
class Evolution:
    ...
    def __init__(self, numFood, numFish):
        ...
        for i in range(0, self.numFish):
            #FISH STATS GO HERE
            self.pond.append(Fish(15, 4, random.randint(0, self.width - 1),
                             random.randint(0, self.height - 1), random.randint(0, 359)))
        #add and draw food
...
e = Evolution(10, 20)  #This is the number of food and number of fish.
                       #Too much food and random movement is encouraged. Too little and they starve.
while True:
    e.tick()
